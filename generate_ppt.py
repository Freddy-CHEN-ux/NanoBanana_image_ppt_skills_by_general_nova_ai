...
def generate_slide(prompt, slide_number, output_dir, resolution="2K"):
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"✗ 第 {slide_number} 页生成失败: HTTP {response.status_code}")
        ...
        if "candidates" not in resp_json:
            print(f"✗ 第 {slide_number} 页生成失败: 响应中没有 candidates")
            print(f"  响应: {json.dumps(resp_json, indent=2)[:500]}")
            return None
        ...
        for part in parts:
            if "inlineData" in part:
                ...
                image_path = os.path.join(output_dir, "images", f"slide-{slide_number:02d}.{ext}")
